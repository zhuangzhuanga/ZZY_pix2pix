...
def train_fn(disc, gen, loader, opt_disc, opt_gen, l1, bce, g_scalar, d_scalar, epoch, clip_model):
    writer = SummaryWriter(log_dir='./23_log/sharp/logs_sharp_train')
...
def eval_fn(gen, loader, epoch, clip_model):
    val_writer = SummaryWriter(log_dir='./23_log/sharp/logs_sharp_test')
    gen.eval()
    loop = tqdm(loader, leave=True)
...
def main():
    ...
    val_target_path = 'datasets/sharp/test'
    val_text_path = 'datasets/text/sharp.txt'
    ...
    for epoch in range(config.NUM_EPOCHS):
        ...
        if config.SAVE_MODEL and epoch % 20 == 0 and epoch > 0:
            filename_gen = '23_model/sharp/sharp_gen_{}.pt'.format(epoch)
            filename_disc = '23_model/sharp/sharp_dis_{}.pt'.format(epoch)
            save_checkpoint(gen, opt_gen, filename_gen)
            save_checkpoint(disc, opt_disc, filename_disc)
        #
        # # generate output on validation data
        # save_some_examples(gen, val_loader, epoch, folder="evaluation/sharpen")

    save_checkpoint(gen, opt_gen, '23_model/final/sharp_generator.pt')
    save_checkpoint(disc, opt_disc, '23_model/final/sharp_discriminator.pt')
