...
def train_fn(disc, gen, loader, opt_disc, opt_gen, l1, bce, g_scalar, d_scalar, epoch):
    writer = SummaryWriter(log_dir='./model_result/logs_gaussian')
...
def main():
    ...
    train_dataset = DeblurDataset('datasets/Gaussian/train')
    train_loader = DataLoader(train_dataset, batch_size=config.BATCH_SIZE, shuffle=True, num_workers=config.NUM_WORKERS)

    g_scalar = torch.cuda.amp.GradScaler()
    d_scalar = torch.cuda.amp.GradScaler()

    val_dataset = DeblurDataset('datasets/Gaussian/val')
    ...
    for epoch in range(config.NUM_EPOCHS):
        ...
        #     save_checkpoint(gen, opt_gen, 'model_result/Gaussian_gen_checkpoint.pt')
        #     save_checkpoint(disc, opt_disc, 'model_result/Gaussian_dis_checkpoint.pt')

        # # generate output on validation data
        # save_some_examples(gen, val_loader, epoch, folder="evaluation/gaussian")
