...
def single_test(img_path, img_name, save_path, gen_path):
    # initialize generator and load pre-trained weights
    gen = Generator(in_channels=3).to(config.DEVICE)
    opt_gen = optim.Adam(gen.parameters(), lr=config.LEARNING_RATE, betas=(0.5, 0.999))
    load_checkpoint(gen_path, gen, opt_gen, config.LEARNING_RATE)

    # load the image
    input_image = np.array(Image.open(os.path.join(img_path, img_name)))
    ...
    with torch.no_grad():
        ...
        save_image(generated_image, os.path.join(save_path, img_name))
...
def batch_test():
    ...
    # Load the generator
    load_checkpoint("save_model/sharpen/Final_generator.pt", gen, opt_gen, config.LEARNING_RATE)

    # Initialize the dataflow
    train_img_path = 'datasets/Sharpen/train'
    train_text_path = 'datasets/Sharpen/train/edit.txt'
    # train_dataset = DeblurDataset(train_img_path, train_text_path)
    test_dataset = DeblurDataset(train_img_path, train_text_path)
...
if __name__ == "__main__":
    # main() if len(sys.argv) == 1: try: single_test("test.jpg") except FileNotFoundError: print("Please make sure
    # there is 'test.jpg' in the root folder") print("Or if you want to specify the file path, run the script from
    # commandline and pass the image filepath(including the filename) as the argument")
    #
    # else:
    #     if sys.argv[1] == "batch":
    #         batch_test()
    ...
    # single_test(img_path, img_name, save_path, gen_path)
    # single_test('test-images', 'sharpen.jpg', 'test_results', 'model_result')
    batch_test()
